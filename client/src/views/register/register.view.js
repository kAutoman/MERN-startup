...
const Register = ({register , isAuthenticated, isDark}) => {
...
//     }
    ...
    return(
        <div className={`${isDark && 'dark'}`}>
            <div className='grid grid-cols-1 min-[1030px]:grid-cols-2 gap-5 px-10 m-auto dark:bg-[#222b45] min-h-[110vh]' id='login'>
                <form className='text-gray-800 m-auto md:m-3' onSubmit={onSubmit}>
                    <p className='text-5xl font-extrabold text-black dark:text-white'>Create your account</p>
                    <p className='text-base font-semibold mt-5 dark:text-white'>Choose your KENEK username, you can always change it later.</p>
                    <div id='form-group'>
                        <div className='flex items-center bg-gray-200  px-4 py-4 rounded-2xl '>
                            <div className='bg-gray-200'>
                                <p className='text-gray-400'>kenek.co/</p>
                            </div>
                            <input type={'text'} placeholder="username" autoComplete="new-password" name='username' value={username} onChange={onChange} className='bg-gray-200 px-2  text-base focus:outline-none w-full'/>
                        </div>
                    </div>
                    <div id='form-group'>
                        <div className='bg-gray-200 text-base  px-4 py-4 rounded-2xl '>
                            <input type={'email'} name='email' value={email} placeholder="Email Address" onChange={onChange} className='bg-gray-200 px-2  text-base focus:outline-none  w-full'/>
                        </div>
                    </div>

                    <div id='form-group'>
                        <div className='flex flex-row justify-between items-center bg-gray-200 text-base  px-4 py-4 rounded-2xl '>
                            <input type={`${passwordShow? 'text ': 'password'}`} id="password_input" name='password' value={password} onChange={onChange} placeholder="Password" className='bg-gray-200 px-2  text-base focus:outline-none  w-full'/>
                    ...
                    {/* submit button */}
                    <div id='submit_button' type="submit" className='mt-10 flex justify-center'>
                        <button className='bg-[rgba(234,180,1,0.83)] px-3 py-4 w-full rounded-2xl hover:bg-[rgb(234,180,1)]'>
                            <span className='text-white text-xl'>Create an Account</span>
                        </button>
                    </div>
                    {/* Already have an account ? redirect : not */}
                    <div id='already_signed' className='mt-5 text-center'>
                        <p className='text-gray-700 font-semibold dark:text-white'>Already create an account ? <span><Link to={'/login'} className="underline text-yellow-500">Login</Link> here</span></p>
                    </div>
                </form>
